...
void DCF77Receive::begin(uint8_t intPin) {
  _intPin = intPin;
  if (_intPin != PIND2 && _intPin != PIND3) {
    return;
  }
  begin();
}
...
/// @param activeLow 
//////////////////////////////////////////////////////////////////////////////
void DCF77Receive::setActiveLow(bool activeLow) {
...
bool DCF77Clock::decodeSequence() {
  ...
  //dcf77Seq->summertime = (int0_dcf77DataBuffer >> 17) & 0x03;       // MEZ: b17=0 b18=1 / MESZ = b17=1 b18=0  bit 17-18
  ...
  _parityBitMinutes = (_sequenceBuffer >> 28) & 0x01;
  _hours = (_sequenceBuffer >> 29) & 0x3F;            // hour = bit 29-34
  _parityBitHours = (_sequenceBuffer >> 35) & 0x01;
  ...
  _parityBitDate = (_sequenceBuffer >> 58) & 0x01;
  ...
  if(_startBit) {
    ...
    //
    if ( (__builtin_parity(_minutes) == _parityBitMinutes) && 
         (__builtin_parity(_hours) == _parityBitHours) ) {
      //
      // It is possible that nonsensical time values will also result in correct parity. 
      // That's why an additional check is made.
      //
      _parityTimeOK = !( 
        abs( bcdToDec(_minutes) - ((bcdToDec(_oldMinutes) + 1) % 60)) || 
        abs( bcdToDec(_hours) - ((bcdToDec(_oldHours) + (_minutes ? 0 : 1)) % 24) ) //Add 1 if an hour change has occurred
      );
    }
...
//////////////////////////////////////////////////////////////////////////////
bool DCF77Clock::getLeapSecond() const {
  return _leapSecond;
}
...
//////////////////////////////////////////////////////////////////////////////
uint8_t DCF77Clock::getSeconds() const {
  return _seconds;
}

uint8_t DCF77Clock::getMinutes() const {
  return bcdToDec(_minutes);
}

uint8_t DCF77Clock::getHours() const {
  return bcdToDec(_hours);
}

uint8_t DCF77Clock::getDay() const {
  return bcdToDec(_dayOfMonth);
}

uint8_t DCF77Clock::getMonth() const {
  return bcdToDec(_month);
}

uint8_t DCF77Clock::getYear() const {
  return bcdToDec(_year);
}

uint8_t DCF77Clock::getBcdMinutes() const {
  return _minutes;
}

uint8_t DCF77Clock::getBcdHours() const {
  return _hours;
}

uint8_t DCF77Clock::getBcdDay() const {
  return _dayOfMonth;
}

uint8_t DCF77Clock::getBcdMonth() const {
  return _month;
}

uint8_t DCF77Clock::getBcdYear() const {
  return _year;
}
